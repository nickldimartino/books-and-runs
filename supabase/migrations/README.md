...
| 0056 | Bonus-XP ledger + quests — `xp_ledger` (`(user_id, ref)` primary key = idempotent credits; service-role-only writes, owner read) for Daily Deal/Weekly Challenge completion XP, streak milestones and quest payouts; `quest_baselines` (per-period metric snapshot the client can read but never write); `my_bonus_xp()`; a service-role-only `solo_verify_set_counters()` jsonb merge; `compute_total_xp()` re-created with the ledger sum. Run BEFORE redeploying `solo-verify`. |
| 0057 | Four Daily/Weekly achievement families (`challenges` category) added to `achievement_thresholds`, with a backfill of `achievement_counters` from existing `daily_deal_completions`/`weekly_challenge_completions` (completion counts + best streaks). Run after 0056. |
| 0073 | Syncs the game-feel/accessibility settings from the 2026-09-25 audit pass: nullable `game_speed`, `reduce_motion`, `show_legal_moves`, `confirm_discard` columns on `settings` (see `accountSettingsSync.ts`). |
| 0080 | Daily/Weekly streak fix + repair — AFTER INSERT triggers on `daily_deal_completions`/`weekly_challenge_completions` refresh the account's `leaderboard_entries` streak columns (previously only recomputed when the client wrote them, racing the server-side completion insert); idempotent repair backfills last-3-days Daily completions that have a `daily_deal_scores` row, recomputes cached streaks and Daily counters. Run after 0073, redeploy `solo-verify` too. |

| 0060 | Safety layer — `user_blocks` + `block_user`/`unblock_user`/`my_blocks` (blocked pairs can't friend, find each other by code, or appear on each other's leaderboard), `user_reports` + `report_user()` (one report queue; `report_profile_photo()` now files into it), and a server-side content filter (`content_blocklist`, `content_flagged()`, triggers on `leaderboard_entries` display_name/bio and club/tournament names; list generated by `scripts/gen-blocklist-sql.mjs` from `src/safety/blocklist.ts`). |
| 0061 | Multiplayer turn clock + emotes — `mp_games.turn_limit_hours`/`turn_started_at`/`turn_warned_at` (existing games get 0 = no limit; new games default 72), `mp_participants.missed_turns`, `mp_my_games()` returns the clock, `mp_emotes` (fixed presets, block-aware RLS, realtime), and an optional 30-minute `pg_cron` sweep of the `mp` function's `/sweep` route. |
| 0062 | Notification preferences — `settings.notify_turns`/`notify_invites`/`notify_nudges`/`notify_streaks`, `quiet_hours_start`/`_end`, `tz_offset_minutes` (synced like every setting; enforced server-side in `_shared/push.ts`). |
| 0063 | Server-side leaderboard — `leaderboard_page()` / `leaderboard_my_row()` (ranking, season delta, friends scope, block + test-account filtering, real rank + total) so the app no longer downloads/sorts the whole table (and no 1000-row truncation). Apply BEFORE deploying the matching client. |
| 0064 | Account deletion support — `delete_account_prepare()` (service role only): revokes pushes, re-homes hosted games, anonymises the account's seats to "Deleted player" in `mp_games.seats` and the sealed engine. Used by the new `delete-account` Edge Function. |
| 0081 | Streak shields — free, earned, automatic grace for the Daily Deal (1 shield per 7-day streak, hold up to 2, covers one missed day) and Weekly Challenge (1 per 4-week streak, hold 1, covers one missed week). Pure SQL walk `streak_shield_walk()` + `daily_deal_shield_state()`/`weekly_challenge_shield_state()`; the 0036/0039 streak-truth triggers now also own the new `leaderboard_entries` shield columns (defaulted; old clients unaffected) and fire on them. Repair recomputes every account from its completion history and refreshes the best-streak counters. Run after 0080; redeploy `solo-verify` too. |
| 0082 | Live-test fixes — drops the removed emote feature (`mp_emotes`), `block_user()` also cancels pending games/invites between the pair, and `mp_bump_rate_limit()` is revoked from `authenticated` (only called inside SECURITY DEFINER RPCs). |
| 0083 | Content filter — `content_words()` also tries a spelled-out run without a glued leading "a"/"i" ("a f u c k"); mirrors `src/safety/contentFilter.ts`. |
| 0084 | `solo_game_hashes` (solo-verify idempotency: a re-submitted verified log no longer counts twice) and `refresh_leaderboard_truth(uid)` (service role; refreshes `leaderboard_entries.total_xp` after server-side XP credits). Run before redeploying `solo-verify`. |
