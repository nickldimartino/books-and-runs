 ...
 */
export function HandPreviewBar({ cards, hidden, onTap, showOnAllScreens }: HandPreviewBarProps) {
  ...
  return (
    ...
    >
      ...
      >
        {cards.map((card, i) => (
          <MiniCard key={card.id} card={card} marginLeft={i === 0 ? 0 : step - CARD_W} fanned={fanned} />
        ))}
      </div>
    </button>
  );
...
function MiniCard({ card, marginLeft, fanned }: { card: Card; marginLeft: number; fanned: boolean }) {
